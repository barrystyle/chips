...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "chipsd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "chips-cli" + config["environment"]["EXEEXT"],
        )
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'chipsd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'chips-cli', self.options.bitcoincli) for v in versions]
    ...
    def skip_if_no_bitcoind_zmq(self):
        ...
        if not self.is_zmq_compiled():
            raise SkipTest("chipsd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        ...
        if not self.is_wallet_tool_compiled():
            raise SkipTest("chips-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("chips-cli has not been compiled.")
